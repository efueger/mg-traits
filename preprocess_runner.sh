...
bbduk.sh in="${R1_raw_all}" in2="${R2_raw_all}" out1="${PE1_rmadapt}" out2="${PE2_rmadapt}" outs="${SE_rmadapt}" qin=33 minlen=45 ktrim=r k=25 mink=11 ref=${TARAADAP} hdist=1 tbo tpe maxns=0 threads=${NSLOTS}
...
########################################################
/bioinf/software/pear/pear-0.9.8/bin/pear -j ${NSLOTS} -y ${MEM} -v ${MINOV} -f "${PE1_rmadapt}" -r "${PE2_rmadapt}" -o "${NAM}.pear"
...
if [[ -s "${RES}/${NAM}.pear.unassembled.forward.fastq" ]]; then
  ...
  bbduk.sh in="${RES}/${NAM}.pear.unassembled.forward.fastq" in2="${RES}/${NAM}.pear.unassembled.reverse.fastq" out1="${PE1_qc_nonmerged}" out2="${PE2_qc_nonmerged}" outs="${RES}/tmp.SR.fasta" qin=33 minlen=45 qtrim=rl trimq=20 ktrim=r k=25 mink=11 ref=${TARAADAP} hdist=1 tbo tpe t=${NSLOTS} maxns=0 threads=${NSLOTS}
...
if [[ -s ${RES}/*assembled.fastq || -s "${SE_rmadapt}" ]]; then
  ...
  bbduk.sh in="${SE_rmadapt}" out1="${SR_qc}" qin=33 minlen=45 qtrim=rl trimq=20 ktrim=r k=25 mink=11 ref=${TARAADAP} hdist=1 tbo tpe t=${NSLOTS} maxns=0 threads=${NSLOTS}
...
rm "${RES}/tmp.SR.fasta" "${SE_rmadapt}" "${PE1_qc_nonmerged}" "${PE2_qc_nonmerged}" ${RES}/*assembled.fastq
